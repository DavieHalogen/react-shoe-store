...
const MyAppBar = () => {
  ...
  return (
    <Box className={classes.appBar} >
        ...
        {isAdmin && <Button sx={{marginLeft: 3,}} variant="contained" color="secondary" onClick={handleAdminNavigate}>{adminButtonLabel}</Button> }
        ...
        {user?.user ? (
         ...
         <>
          ...
          <div className={classes.userName}>
            <Typography 
              sx={{ display: { xs: 'none', sm: 'block' }}}  variant="h6">{user?.user.username}</Typography>
          </div>
